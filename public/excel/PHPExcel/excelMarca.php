...
// Add some data
$tituloReporte = "Familia Goleadora Reporte ";
...
$max="SELECT participante.cedula, participante.nombre, participante.apellido, ciudad.nombre AS ciudadId, participante.telefono, participante.celular, participante.email, count(participanteProducto.participanteCedula) as cant_prod, participante.created_at FROM participante JOIN participanteProducto ON participante.cedula = participanteProducto.participanteCedula JOIN ciudad ON participante.ciudadId = ciudad.id GROUP BY participanteProducto.participanteCedula ORDER BY cant_prod DESC";
...
if($totalpuntos)
{
    for ($f=0; $f < $total; $f++) {
        ...
        ->setCellValue('D'.$i,   $totalpuntos[$f]['ciudadId'])
        ->setCellValue('E'.$i,   $totalpuntos[$f]['telefono'])
        ->setCellValue('F'.$i,   $totalpuntos[$f]['celular'])
        ->setCellValue('G'.$i,   $totalpuntos[$f]['email'])
        ->setCellValue('H'.$i,   $totalpuntos[$f]['cant_prod'])
        ->setCellValue('I'.$i,   $totalpuntos[$f]['created_at']);
