...
->setDescription("Gatos Rugby Club.")
->setKeywords("office 2007 openxml php")
->setCategory("Gatos Rugby Club");
function getArgs(){
...
// Add some data
$tituloReporte = "Gatos Rugby Club Reporte Total Usuarios ";
...
$max="SELECT user.identificacion,user.username,user.email,perfil.nombre AS group_id,estado.nombre AS id_estado,user.foto,user.created_at FROM user
JOIN perfil ON user.group_id = perfil.id_perfil
JOIN estado ON user.status = estado.id_estado";
...
foreach ($DatosUsuario as $key => $value) {
    ...
    $cont2 = $cont2 + 1;
    array_push($totalpuntos, array('cedula'=> $value['identificacion'],
...
if($totalpuntos)
{
    for ($f=0; $f < $total; $f++) {
        ...
        ->setCellValue('B'.$i,   $totalpuntos[$f]['username'])
        ->setCellValue('C'.$i,   $totalpuntos[$f]['email'])
        ->setCellValue('D'.$i,   $totalpuntos[$f]['group_id'])
        ->setCellValue('E'.$i,   $totalpuntos[$f]['id_estado'])
        ->setCellValue('F'.$i,   $totalpuntos[$f]['foto'])
        ->setCellValue('G'.$i,   $totalpuntos[$f]['created_at']);
...
$objPHPExcel->getActiveSheet()->setTitle('Reporte Gatos Rugby Club');
